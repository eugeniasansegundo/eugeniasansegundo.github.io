...
function allScales() {
    ...
    this.crossOv_src = '00.gif';   //default image source of cross for slides on drag
...
function printScale(id) {
    ...
    document.writeln('<img id="tick' + id + '" name="tick' + id + '" src="' + scales.items[id].tick + '" width="' + this.cross_width + '" height="' + this.cross_height + '" alt="" />');
}
...
function setCrosses() {
    for(var i = 0; i < this.items.length; i++) {
        ...
        /* position tick */
        dd.elements["tick"+i].moveTo((dd.elements["line"+i].x + (this.items[i].width / 2) - halfCrossWidth(i)), crossY(i));
        dd.elements["line"+i].addChild("tick"+i);
        dd.elements["tick"+i].defx = dd.elements["line"+i].x + (this.items[i].width / 2) - halfCrossWidth(i);

        if(this.items[i].mode == 'click') dd.elements["line"+i].maximizeZ();
        else dd.elements["tick"+i].maximizeZ();
    }
}
...
function my_PickFunc() {    
    ...
    /* swap image of slider on drag */
        ...
        //the image was not hidden when moving, so I decided to comment this line
    }
}
